...
function renderSessionState(nextAuthenticated) {
  ...
  if (nextAuthenticated) {
    ...
    primary.href = primary.dataset.dashboardHref || `${appOrigin}/app/dashboard`;
...
async function registerRefreshWorker() {
  ...
  try {
    const registration = await navigator.serviceWorker.register(
      `${siteBase}account-session-sw.js`,
      { scope: siteBase },
    );
    ...
    // Periodic Background Sync is optional. Foreground recovery remains.
  }
}
