...
test('Astro registers the Fiducia loader at the gateway base path', async () => {
  const config = await source('astro.config.mjs');
  assert.match(config, /oresWasmLoader/);
  assert.match(config, /appId:\s*['"]fiducia-cloud['"]/);
  assert.match(config, /PUBLIC_BASE/);
});
test('bootstrap pins verified external inputs and remains intent-only', async () => {
...
test('static output contains the base-aware page bootstrap', async () => {
  ...
  assert.match(output, /fiducia\/owls\/marketing-loader\.mjs|owls\/marketing-loader\.mjs/);
